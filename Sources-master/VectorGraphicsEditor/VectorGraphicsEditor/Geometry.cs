...
namespace test_editor
{
    ...
    public class Figure : IFigure
    {
        ...
        IEnumerable<Point> _pointsFromGUI { get; set; } //точки, приходящии от ГУИ, если и использовать, то с типом фигуры)
        ...
        public Figure(string type, ref List<Point> guiPoints )
        {
            ...
            _figureBorder = _pointsFromGUI.ToArray<Point>();
            //
            //CreatePaths();
            //Настя, сюда запихай выпуклую оболочку вместо следующей строки
            _convexHull = _pointsFromGUI.ToArray<Point>();
        ...
        {   //просто одно ребро, потом понадобится
            ...
            DeleteBoundaryPoints:
                ...
                while (innerPoints.Count != 0)
                {
                    ...
                    int indexExtTriangle = _triangles.FindIndex(x => IsPointInTriagle(considered, x, out edge) == "Inner");
